...
Game::Game(sf::RenderWindow *window)
        : windows(*window), gameState(GAME_STATE::MAIN_MENU), playerClass(PLAYER_CLASS::WARRIOR), screenCenter({0, 0}), isRunning(true), end(false), levelWasGenerated(false), keyTimeMax(0.3), goldTotal(0), projectileTextureID(0) {
    // Enable VSync.
...
float Game::DistanceBetweenPoints(sf::Vector2f position1, sf::Vector2f position2) {
    return (std::abs(static_cast<float>(sqrt(((position1.x - position2.x) * (position1.x - position2.x)) +((position1.y - position2.y) * (position1.y - position2.y))))));
}
            ...
            if (playerTile.type == TILE::WALL_DOOR_UNLOCKED) {
                ...
                if (player->IsAttacking()) {
                               ...
                               player->GetClass() == PLAYER_CLASS::WIZARD)//mana cost
                    {
                        ...
                        std::unique_ptr<Projectile> proj = std::make_unique<Projectile>(TextureManager::GetTexture(projectileTextureID), player->GetPosition(),player->GetPosition(),target);
